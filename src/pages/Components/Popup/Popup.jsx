...
function Popup(props) {
  ...
  const [shouldInsertACustomerData, setShouldInsertACustomerData] =
    useState(false)
  const [shouldFindANewCustomerID, setShouldFindANewCustomerID] =
    useState(false)
  ...
  const [date, setDate] = useState(
    new Date(2023, 3, 9).toLocaleDateString("en-CA", { timeZone: "UTC" })
  )
    ...
    if (shouldFetchCustomerData) {
      const getData = async (url) => {
        ...
        if (results.result.length == 1) {
          ...
          props.employeeDataFunction(null)
          //console.log(customerData)
          setWasLoginSuccessful(true)
          props.isCustomerLoggedInFunction(true)
      ...
      getData(
        "http://localhost:3000/api/SelectData/SelectAllCustomerInfoIfTheyExist?userName=" +
          emailInput +
          "&userPassword=" +
          passwordInput
      )
    ...
    if (shouldFetchEmployeeData) {
      const getData = async (url) => {
        ...
        if (results.result.length == 1) {
          ...
          props.customerDataFunction(null)
          setWasLoginSuccessful(true)
          props.isEmployeeLoggedInFunction(true)
      ...
      getData(
        "http://localhost:3000/api/SelectData/SelectAllEmployeeInfoIfTheyExist?userName=" +
          emailInput +
          "&userPassword=" +
          passwordInput
      )
